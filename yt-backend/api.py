...
@api.get("/charts")
def get_chart(brand: str) -> list[Chart]:
    sentiment = requests.post(ML_URL, data=json.dumps({
        "teams": [
            {
                "brand": brand,
                "texts": [f"{brand} cool", f"{brand} sucks"]
            }
        ]
    })).json()
    print(sentiment)
    ...
    return [
        Chart(title="Sample chart.", plotly_json=json.loads(json_fig_str))
    ]
